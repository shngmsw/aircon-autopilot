...
- 室温 < `FREE_COOL_ABORT_ROOM`（既定29.0℃）
- 冷房復帰直後のロックアウト中でない

冷房へ戻る条件:

- 室温が `FREE_COOL_ABORT_ROOM`（既定29.0℃）以上になったとき。以後 `FREE_COOL_LOCKOUT_MIN`
  （既定90分）は送風へ戻らないので、送風と冷房を行ったり来たりしません
...
| `FREE_COOL_ABORT_ROOM` | `29.0` | この室温以上で冷房へ復帰（℃） |
| `FREE_COOL_LOCKOUT_MIN` | `90` | 復帰後、再突入を禁止する時間（分） |
