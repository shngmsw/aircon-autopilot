...
- 設定温度は冷房と対称に、不足分 × `ROOM_TARGET_GAIN` だけ上げます。ただし目標帯の上限（`ROOM_TARGET_HIGH`）を超えません。暖房は設定温度を上げすぎると暑くなりすぎるためです。機種の対応温度への丸めも暖房は切り捨てです（25.5℃なら25℃）
...
| `ROOM_TARGET_SET_MAX` | `30.0` | 設定温度の上限（℃）。暖房時は目標帯の上限とこの値の低い方が設定温度の上限 |
