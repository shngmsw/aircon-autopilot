...
- 室温が `ROOM_TARGET_LOW` を下回った → 冷房を止める
- 目標帯の中 → 設定はそのまま
...
室温が目標帯の下限を下回り、かつ外気温が `HEAT_OUT_MAX`（既定 20℃）より低ければ暖房（`warm`）する。設定温度は冷房と対称に「不足分 × gain」だけ上げる。室温が帯に入ったら（下限 + `HYSTERESIS`、ただし上限を超えない）電源を切り、再び下限を下回ったら暖房し直す。暖房中に室温が上限を超えた場合も一旦電源を切る（冷房が要るかは次のサイクルで判断する）。外気温の条件は、夏の朝など「たまたま室温が帯を下回った」だけで暖房しないためのガード。暖房非対応の機種では電源オフにフォールバックする。
